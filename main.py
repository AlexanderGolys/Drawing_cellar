...
class Color:
    ...
    def random_gray():
        ...
        Return:
            tuple[int]: R, G and B chanel
        """
        value = random.randint(0xDD, 0xFF)
...
class Shelf:
    ...
    """
    @staticmethod
    def bool_bernoulli_picker(p):
        if 0 > p or p > 1:
            raise ValueError("Parameter p must be from range <0, 1>.")
    ...
    def __init__(self, w, h, coords, color):
        ...
        Raises:
            ArithmeticError: when probabilities of items listed in choice_list don't sum to 1.
        """
        ...
        choice_list = [Jar, Salt, Beer]
        sum_ = 0
        for item in choice_list:
            sum_ += item.probability
        if not math.isclose(sum_, 1):
            raise ArithmeticError("Probabilities of listed items do not sum to 1.")

        for length in item_widths:
            for item in choice_list:
                chosen = item
                if Shelf.bool_bernoulli_picker(item.probability):
                    break
            self.items.append(chosen(Coords(*item_lb_coords),
                                   int(round(length * self.width)),
                                   self.height - GlobalSetup.rack_thickness))
            item_lb_coords = (item_lb_coords[0] + round(length * self.width), item_lb_coords[1])
...
class Jar(AbstractItem):
    ...
    def __init__(self, lb_coords, w, h):
        ...
        self.h = random.randint(self.h//2, 9*self.h//10)
    ...
    def draw(self):
        ...
        # lid
        w = self.w - 2*self.w//10   # Better alignment (still not perfect unfortunately)
        ...
        # label
        w = self.w - 2*self.w//4   # Better alignment (still not perfect unfortunately)
...
class Beer(AbstractItem):
    ...
    def __init__(self, lb_coords, w, h):
        ...
        if self.h > 2 * self.w:
            ...
            self.w = new_w
        self.h -= 2     # looks bad with max height
    ...
    def draw(self):
        ...
        w = (2*self.w)//3 - self.w//3   # cause caps wasn't perfectly aligned with the bottle neck
                                        # (sometimes it was shifted by 1 pixel)

        if ran == 0:
            DrawingMethods.draw_rectangle(self.lb_coords.add_unbiased((self.w//3, 11 * self.h//12)).undo_shift(),
                                         (w, self.h//12), self.cap_color, self.cap_color)
        else:
            DrawingMethods.draw_rectangle(self.lb_coords.add_unbiased((self.w // 3, 11 * self.h // 12)).undo_shift(),
                                          (w, self.h // 12), self.label_color, self.label_color)
